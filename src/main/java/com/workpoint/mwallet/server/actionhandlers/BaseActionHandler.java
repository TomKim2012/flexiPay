...
/**
 * This is a super class for all requests handlers. It is meant to help with
 * management of transactions, and error handling
 * 
 * 
 * @author duggan
 * 
...
public abstract class BaseActionHandler<A extends BaseRequest<B>, B extends BaseResponse>
	...
	public final B execute(A action, ExecutionContext execContext)
		...
		log.debug(action.getRequestCode() + ": Executing command "
				+ action.getClass().getName());

		if (SessionHelper.getHttpRequest() != null) {
			// embedded call -- needed when executing multiple commands in one
			// call
	...
	private void logErrors(boolean hasError, Throwable throwable, B result) {
		...
		if (hasError) {
			...
			try {
				...
				errorId = ErrorLogDaoHelper.saveLog(throwable, this
			...
			if (throwable.getMessage() == null) {
				baseResult
						.setErrorMessage("An error occurred during processing of your request");
			} else {
				log.error("Throwable : " + throwable.getMessage());
				if (throwable instanceof ConstraintViolationException) {
					baseResult
							.setErrorMessage("[400] A database error occurred");
				} else if (throwable instanceof PersistenceException) {
					baseResult
							.setErrorMessage("[500] A database error occurred and has been logged");
					...
					baseResult
							.setErrorMessage("We could not complete your request due to an error: "
